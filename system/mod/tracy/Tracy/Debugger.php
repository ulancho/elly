...
class Debugger
{
	...
	public static $maxLen = 550;

	/** @var bool display location? {@link Debugger::dump()} */
	public static $showLocation = FALSE;

	/** @var array of variables, that need to go in ajax tab */
	public static $ajaxDumpVars = array();

	public static $currentContext = 'html';
	...
	public static function getBar()
	{
		if (!self::$bar) {
			...
			self::$bar->addPanel(new DefaultBarPanel('sql'), __CLASS__ . ':sql'); // filled by sql()
			self::$bar->addPanel(new DefaultBarPanel('ajax'), __CLASS__ . ':ajax'); // filled by print_arr() for ajax
			self::$bar->info = array(
				'PHP ' . PHP_VERSION,
				isset($_SERVER['SERVER_SOFTWARE']) ? $_SERVER['SERVER_SOFTWARE'] : NULL,
				'Tracy ' . self::$version,
	 ...
	 * @return mixed  variable itself
	 */
	public static function ajax($var, $title = NULL, array $options = NULL)
	{
		if (!self::$productionMode) {
			self::getBar()->getPanel(__CLASS__ . ':ajax')->data[] = array('title' => $title, 'dump' => Dumper::toHtml($var, (array) $options + array(
				Dumper::DEPTH => self::$maxDepth,
				Dumper::TRUNCATE => self::$maxLen,
				Dumper::LOCATION => self::$showLocation,
			)));
	 ...
	 * @return mixed  variable itself
	 */
	public static function sql($var, $title = NULL, array $options = NULL)
	{
		if (!self::$productionMode) {
			self::getBar()->getPanel(__CLASS__ . ':sql')->data[] = array('title' => $title, 'dump' => $var);
			// self::getBar()->getPanel(__CLASS__ . ':sql')->data[] = array('title' => $title, 'dump' => Dumper::toHtml($var, (array) $options + array(
			// 	Dumper::DEPTH => self::$maxDepth,
			// 	Dumper::TRUNCATE => self::$maxLen,
			// 	Dumper::LOCATION => self::$showLocation,
			// )));
