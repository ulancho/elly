...
function EventManager(options, _sources) {
	...
	t.removeAnnotations = removeAnnotations;
	...
	var stickyAnn = { annotations: [] };
	var sources_ann = [ stickyAnn ];
	...
	function _fetchEventSource(source, callback) {
		...
		if (events) {
			...
			if (url) {
				...
				pushLoading();
				data.date_start = data[startParam];
				data.date_end = data[endParam];
				$('#filter_date_start').val(data.date_start);
				$('#filter_date_end').val(data.date_end);
...
/*
	function reportAnn(annotations) { // events are already normalized at this point
		annByID = {};
		var i, len=annotations.length, annotation;
		for (i=0; i<len; i++) {
			annotation = annotations[i];
			if (annByID[annotation._id]) {
				annByID[annotation._id].push(annotation);
			}else{
				annByID[annotation._id] = [annotation];
			}
		}
	}*/
...
function HorizontalPositionCache(getElement) {
	...
	t.left = function(i) {
		return lefts[i] = lefts[i] === undefined && e(i).position() ? e(i).position().left : lefts[i];
	};
