...
TimeGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		return '<a class="' + classes.join(' ') + '"' +
			...
			'>' +
				'<div class="fc-content">' + 
                (event.check_pay ? '<img src="img/icons/money-coin.png" style="width:12px;"> ' : '' ) +
					(timeText ?
						'<div class="fc-time"' +
	...
	function _fetchEventSource(source, callback) {
		...
		if (events) {
			...
			if (url) {
				...
				$('#filter_date_start').val(data.start);
				$('#filter_date_end').val(data.end);

				// sendFilterValues();
				/* Находит докторов которые checked  */
				var code_doctors = $("#content-tree input.check_doctor:checkbox:checked").map(function(){
					return $(this).attr('data-code_doc');
				}).get();

				$('#filter_code_doctor').val(code_doctors);
				code_doctors = $('#filter_code_doctor').val();

				if( (Object.prototype.toString.call( code_doctors ) === '[object Array]' && code_doctors.length==1) || typeof(code_doctors)=='string'){
					var code_department = $("#content-tree input.check_doctor:checkbox:checked").map(function(){
						return $(this).parents('li').find('input.check_department').attr('data-code_dep');
					}).get();
				} else {
					var code_department = 0;
				}

				data.code_doctor = code_doctors;
				data.code_department = code_department;

				EllyCore.ajax($.extend({}, ajaxDefaults, source, {
					url: url,
					data: data,
					success: function(events) {
						if(data.doctor_freetime){
							$.each(data.doctor_freetime, function(index, element) {
								data.doctor_freetime[index].start = new Date(data.doctor_freetime[index].start*1000);
								data.doctor_freetime[index].end = new Date(data.doctor_freetime[index].end*1000);
							});
							annotations = data.doctor_freetime;
						} else {
							annotations = [];
						}

						$('#calendar').fullCalendar('removeEvents');
						events = events.week_events || [];
